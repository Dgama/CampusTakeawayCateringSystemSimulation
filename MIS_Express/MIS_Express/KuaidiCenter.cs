...
{
    public partial class KuaidiCenter : Form
    {
        string name_id;
        string id;

        public KuaidiCenter(string name_id,string id)
        {
            ...
            快递员信息ToolStripMenuItem.Image = Image.FromFile(@"Pic&Ico\profile.ico");
            待配送ToolStripMenuItem.Image = Image.FromFile(@"Pic&Ico\checklist.ico");
            待发货ToolStripMenuItem.Image = Image.FromFile(@"Pic&Ico\outbox.ico");
            揽件分配ToolStripMenuItem.Image = Image.FromFile(@"Pic&Ico\order.ico");
        ...
        private void 在途快递ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            + "' and 寄件人.寄件人ID = 订单.寄件人ID and 收件人.收件人ID = 订单.收件人ID and 订单.地址表ID = 当前地址表.地址表ID and 当前地址表.所属点ID =快递点and集散中心表.所属点ID and 订单.用户账号 =用户登录信息表.用户账号 and 订单.揽件时间 is not null and 当前地址表.发出时间 is null and 当前地址表.到达时间 is null and 快递点and集散中心表.类型 = 0 and 收件人.地址 = 地址.地址ID";
            SA = new SqlDataAdapter(Sql, con);
            SA.Fill(DS, "在途快递");
            this.dataGridView2.DataSource = DS.Tables["在途快递"];
        ...
        private void 快递员信息ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            panel1.Controls.Clear();
            ...
            SqlDataAdapter SA;
            DataSet DS = new DataSet();
            string Sql = "select 快递员.快递员ID, 快递员.姓名, 快递员.性别, 快递员.电话  "
            + "  from 快递员,快递点and集散中心表"
            + "  where 快递点and集散中心表.所属点ID = '" + id
            + "' and 快递员.所属点ID = 快递点and集散中心表.所属点ID";
            SA = new SqlDataAdapter(Sql, con);
            SA.Fill(DS, "快递员信息");
            this.dataGridView4.DataSource = DS.Tables["快递员信息"];
        ...
        {
            ChangeCode changeCode = new ChangeCode(id,3);
        ...
        private void 待配送ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            panel1.Controls.Clear();
            ...
            string sql = string.Format("select 姓名 from 快递员 where 所属点ID = '{0}'",id);
            DataSet DS = new DataSet();
            SA = new SqlDataAdapter(sql, con);
            SA.Fill(DS, "sheet");
            int manRows = DS.Tables[0].Rows.Count;
            for (int i = 0; i < manRows; i = i + 1)
            {
                comboBox4.Items.Add(DS.Tables["sheet"].Rows[i][0].ToString());
            }
            SqlDataAdapter SA2;
            DataSet DS2 = new DataSet();
            string Sql = "select 订单.订单编号,寄件人.姓名 as 寄件人,收件人.姓名 as 收件人,快递点and集散中心表.点位 as 当前地址,用户登录信息表.姓名 as 用户名,订单.重量,订单.物品类型,订单.重要物品,订单.揽件时间,当前地址表.到达时间,当前地址表.发出时间, 订单.收件人取货方式,地址.省,地址.市,地址.区,地址.详细地址 "
            + "  from 订单, 寄件人, 收件人, 快递点and集散中心表, 用户登录信息表, 当前地址表,地址"
            + "  where 快递点and集散中心表.所属点ID = '" + id
            + "' and 寄件人.寄件人ID = 订单.寄件人ID and 收件人.收件人ID = 订单.收件人ID and 订单.地址表ID = 当前地址表.地址表ID and 当前地址表.所属点ID = 快递点and集散中心表.所属点ID and 订单.用户账号 = 用户登录信息表.用户账号 and 当前地址表.到达时间 != 订单.揽件时间  and 快递点and集散中心表.类型 = 0 and 订单.快递点 != 快递点and集散中心表.所属点ID and 订单.收件人取货方式 is null and 订单.是否完成=0 and 收件人.地址 = 地址.地址ID";
            SA2 = new SqlDataAdapter(Sql, con);
            SA2.Fill(DS2, "待配送未分配快递员快递");
            this.dataGridView1.DataSource = DS2.Tables["待配送未分配快递员快递"];

            SqlDataAdapter SA1;
            DataSet DS1 = new DataSet();
            string Sql1 = "select 订单.订单编号,寄件人.姓名 as 寄件人,收件人.姓名 as 收件人,快递点and集散中心表.点位 as 当前地址,用户登录信息表.姓名 as 用户名,订单.重量,订单.物品类型,订单.重要物品,订单.揽件时间,当前地址表.到达时间,当前地址表.发出时间, 订单.收件人取货方式 ,地址.省,地址.市,地址.区,地址.详细地址"
            + "  from 订单, 寄件人, 收件人, 快递点and集散中心表, 用户登录信息表, 当前地址表,地址"
            + "  where 快递点and集散中心表.所属点ID = '" + id
            + "' and 寄件人.寄件人ID = 订单.寄件人ID and 收件人.收件人ID = 订单.收件人ID and 订单.地址表ID = 当前地址表.地址表ID and 当前地址表.所属点ID = 快递点and集散中心表.所属点ID and 订单.用户账号 = 用户登录信息表.用户账号 and 当前地址表.到达时间 != 订单.揽件时间  and 快递点and集散中心表.类型 = 0 and 订单.快递点 != 快递点and集散中心表.所属点ID and 订单.收件人取货方式 is not null and 订单.是否完成=0 and 收件人.地址 = 地址.地址ID";
            SA1 = new SqlDataAdapter(Sql1, con);
            SA1.Fill(DS1, "待配送分配快递员快递");
            this.dataGridView6.DataSource = DS1.Tables["待配送分配快递员快递"];


        }

        private void 待发货ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            panel1.Controls.Clear();
            ...
            + "  from 订单, 寄件人, 收件人, 快递点and集散中心表, 用户登录信息表, 当前地址表,地址"
            + "  where 快递点and集散中心表.所属点ID = '" + id
            + "' and 寄件人.寄件人ID = 订单.寄件人ID and 收件人.收件人ID = 订单.收件人ID and 订单.地址表ID = 当前地址表.地址表ID and 当前地址表.所属点ID = 快递点and集散中心表.所属点ID and 订单.用户账号 =用户登录信息表.用户账号 and 当前地址表.发出时间 is null and 当前地址表.到达时间 is not null and 快递点and集散中心表.类型 = 0 and 订单.快递点 = 快递点and集散中心表.所属点ID and 订单.是否完成=0 and 收件人.地址 = 地址.地址ID";
            SA = new SqlDataAdapter(Sql, con);
            SA.Fill(DS, "代发送库存快递");
            this.dataGridView5.DataSource = DS.Tables["代发送库存快递"];
        ...
        private void KuaidiCenter_Load(object sender, EventArgs e)
        {
        ...
        {
            string dz = this.dataGridView4.CurrentRow.Cells["快递员ID"].Value.ToString();
            Responsibility responsibility = new Responsibility(dz);
            responsibility.ShowDialog();
        }
        ...
        private void 揽件分配ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            panel1.Controls.Clear();
            ...
            for (int i = 0; i < manRows; i = i + 1)
            {
                comboBox5.Items.Add(DS.Tables["sheet"].Rows[i][0].ToString());
            }
            SqlDataAdapter SA2;
            DataSet DS2 = new DataSet();
            string Sql = "select 订单.订单编号,寄件人.姓名 as 寄件人,收件人.姓名 as 收件人,快递点and集散中心表.点位 as 当前地址,用户登录信息表.姓名 as 用户名,订单.重量,订单.物品类型,订单.重要物品, 订单.收件人取货方式 "
            + "  from 订单, 寄件人, 收件人, 快递点and集散中心表, 用户登录信息表"
            + "  where 快递点and集散中心表.所属点ID = '" + id
            + "' and 寄件人.寄件人ID = 订单.寄件人ID and 收件人.收件人ID = 订单.收件人ID and 订单.用户账号 =用户登录信息表.用户账号 and 快递点and集散中心表.类型 = 0 and 订单.快递点=快递点and集散中心表.所属点ID and 订单.揽件人ID is null and 订单.是否完成=0";
            //MessageBox.Show (Sql);
            SA2 = new SqlDataAdapter(Sql, con);
            SA2.Fill(DS2, "待揽件未分配快递员快递");
            this.dataGridView8.DataSource = DS2.Tables["待揽件未分配快递员快递"];

            SqlDataAdapter SA1;
            DataSet DS1 = new DataSet();
            string Sql1 = "select 订单.订单编号,寄件人.姓名 as 寄件人,收件人.姓名 as 收件人,快递点and集散中心表.点位 as 当前地址,用户登录信息表.姓名 as 用户名,订单.重量,订单.物品类型,订单.重要物品, 订单.收件人取货方式 "
            + "  from 订单, 寄件人, 收件人, 快递点and集散中心表, 用户登录信息表"
            + "  where 快递点and集散中心表.所属点ID = '" + id
            + "' and 寄件人.寄件人ID = 订单.寄件人ID and 收件人.收件人ID = 订单.收件人ID  and 订单.用户账号 =用户登录信息表.用户账号 and 快递点and集散中心表.类型 = 0 and 订单.快递点=快递点and集散中心表.所属点ID and 订单.揽件人ID is not null and 订单.是否完成=0";
            SA1 = new SqlDataAdapter(Sql1, con);
            SA1.Fill(DS1, "待揽件分配快递员快递");
            this.dataGridView7.DataSource = DS1.Tables["待揽件分配快递员快递"];
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MessageBox.Show("无相关内容", "消息通知", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        private void comboBox3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    comboBox6.Items.Add(DSSpotNear.Tables["SheetSpotNear$"].Rows[i][0].ToString());
                }*/
                for (int i = 0; i < OtherRows; i = i + 1)
                {
                    comboBox6.Items.Add(DSSpotOther.Tables["SheetSpotOther$"].Rows[i][0].ToString());
                }
            }
            catch
            {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            string ddbh = this.dataGridView5.CurrentRow.Cells["订单编号"].Value.ToString();
            string next_id = "";
            string sql_next_id = string.Format("select 所属点ID from 快递点and集散中心表 where 省 = '{0}' and 市 = '{1}' and 区 = '{2}' and 点位 ='{3}' and  类型 = 1",comboBox1.Text, comboBox2.Text, comboBox3.Text, comboBox6.Text);
            SqlDataAdapter SA1;
            DataSet DS1 = new DataSet();
            SA1 = new SqlDataAdapter(sql_next_id, con);
            SA1.Fill(DS1, "sheet");
            next_id = DS1.Tables["sheet"].Rows[0][0].ToString();
            //MessageBox.Show(next_id);
            ...
            + "') ";*/
            SA = new SqlDataAdapter(Sql, con);
            //SqlCommandBuilder thisBuilder = new SqlCommandBuilder(SA);
            //SA.Update(DS, "更新时间和地址");
            SA.Fill(DS, "更新时间和地址");
            con.Close();
            this.dataGridView5.DataSource = DS.Tables["更新时间和地址"];
            //修改当前行单元格的内容}
        }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            + "  from 订单, 寄件人, 收件人, 快递点and集散中心表, 用户登录信息表, 当前地址表"
            + "  where 快递点and集散中心表.所属点ID = '" + id
            + "' and 寄件人.寄件人ID = 订单.寄件人ID and 收件人.收件人ID = 订单.收件人ID and 订单.地址表ID = 当前地址表.地址表ID and 当前地址表.所属点ID =快递点and集散中心表.所属点ID and 订单.用户账号 =用户登录信息表.用户账号 and 订单.揽件时间 is not null and 当前地址表.发出时间 is null and 当前地址表.到达时间 is null and 快递点and集散中心表.类型 = 0";
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            SqlDataAdapter SA;
            DataSet DS = new DataSet();
            string ddbh = this.dataGridView8.CurrentRow.Cells["订单编号"].Value.ToString();
            string Sql = "update 订单 set 揽件人ID = (select 快递员.快递员ID  from 快递员 where 快递员.姓名='" + comboBox5.SelectedItem.ToString()
            + "')  where 订单编号 = '" + ddbh
            + "'";
            SA = new SqlDataAdapter(Sql, con);
            SA.Fill(DS, "配送上门");
            con.Close();
            this.dataGridView8.DataSource = DS.Tables["配送上门"];
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            if (radioButton5.Checked)
            {
                ...
                string Sql = "update 订单 set 收件人取货方式 = '配送上门' , 快递员ID = (select 快递员.快递员ID  from 快递员 where 快递员.姓名='" + comboBox4.SelectedItem.ToString()
                + "') where 订单编号 = '" + ddbh
                + "  '";
                SA = new SqlDataAdapter(Sql, con);
                SA.Fill(DS, "配送上门");
                con.Close();
                this.dataGridView1.DataSource = DS.Tables["配送上门"];
            }
            else if (radioButton6.Checked)
            {
                ...
                DataSet DS = new DataSet();
                string ddbh = this.dataGridView1.CurrentRow.Cells["订单编号"].Value.ToString();
                string Sql = "update 订单 set 收件人取货方式 = '快递点自取' , 是否完成 = 1"
                + " where 订单编号 = '" + ddbh
                + "  '";
                SA = new SqlDataAdapter(Sql, con);
                SA.Fill(DS, "快递点自取");
                con.Close();
                this.dataGridView1.DataSource = DS.Tables["快递点自取"];
            }
